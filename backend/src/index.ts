...
const render = (
  ...
  const filename = `../html/${
    filebasename.includes(".") ? filebasename : `${filebasename}.html`
  }`;
  ...
  if (filename.endsWith(".html")) {
    ejs.renderFile(filename, data || {}, {}, (err, html) => {
      if (err) {
        console.log(
          `ejs.renderFile ERROR for ${filename}: ${JSON.stringify(err)}`,
        );
        console.log(err);
...
if (config.faucetPassword) {
  ...
  visitorCount = async (req: Request) => {
    const rl: rlf.RateLimiterRes | null = await rlfFaucet.get(
...
app.get("/:filename", (req: Request, res: Response) =>
  get(req, res, req.params.filename),
);
...
const makeClaim = async (
  ...
  if (amount < 100000 || amount > 10000000)
    return res.status(400).send("Please check your amount and try again.");
  let count = await visitorCount(req);
  const requireCaptcha = count > 0;
  if (requireCaptcha) {
    if (req.session.captcha) {
      if (req.session.captcha === "reload")
        return res.status(400).send("Captcha expired; please reload");
  ...
  bitcoin.sendToAddress(address, sat2BTC(amount), async (err, result) => {
    console.log(
      `send ${amount} sats = ${sat2BTC(
        amount as number,
      )} BTC to ${address} ${JSON.stringify(err)} ${JSON.stringify(result)}`,
    );
    if (err) throw new Error("Internal error");
    await claim.record(new Date().getTime(), amount as number);
    res.send(
      `Payment of ${sat2BTC(amount as number)} BTC sent with txid ${
        result!.result
      }`,
    );
  });
...
app.post("/claim", (req: Request, res: Response) =>
  makeClaim(req.body, req, res),
);

if (config.faucetPassword) {
  app.get("/claim/:address/:amount/:password", (req: Request, res: Response) =>
    makeClaim(req.params, req, res),
  );
} else {
  app.get("/claim/:address/:amount", (req: Request, res: Response) =>
    makeClaim(req.params, req, res),
  );
  app.get("/claim/:address/:amount/:captcha", (req: Request, res: Response) =>
    makeClaim(req.params, req, res),
  );
}
